"""按名字加载引擎插件（composition root 的一部分）。

远端 conda 环境与其他项目共用、约定不改动，所以不用 entry point / pip 安装，而是
``EngineSpec(factory="包.模块:函数", root=import 根, kwargs)``：把 root 放进 sys.path 再导入。
各仓库根目录即包（``ResNet`` / ``Transformer`` / ``SSM``），root 是它们共同的父目录
（远端 ``/home/jeefy/UniChess``；开发副本 ``/tmp/rebuild``）。

吸取 R ``_import_isolated`` 的教训：T 和 R 以前都有顶层包 ``core`` / ``model`` / ``search``，
同进程加载时后来者拿到先到者的模块，**无声地算错**（64 token 编码当 19 平面用）。
现在顶层包就是仓库名，不再冲突；这里再加一道保险：导入后核对模块文件确实位于 root 之下，
并拒绝已从别处加载的同名顶层包（例如同进程里生产副本与 /tmp 开发副本混用）。
...
class EngineSpec:
    factory: str                       # "ResNet.kit:make_player_factory"
    root: Optional[str] = None         # import 根（各仓库的父目录，放进 sys.path）
